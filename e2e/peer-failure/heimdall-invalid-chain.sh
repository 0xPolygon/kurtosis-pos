...
_bor_up() {
  local url
  url="$(rpc_url "$TARGET_BOR" 2> /dev/null)" || return 1
...
main() {
  ...
  # OUTSIDE the kurtosis-pos package tree — kurtosis fails to resolve a nested sub-package.
  kurtosis service rm "$ENCLAVE" "$PROXY_SVC" > /dev/null 2>&1 || true
  local pkgtmp
  pkgtmp="$(mktemp -d)"
  ...
  if ! kurtosis run --enclave "$ENCLAVE" "$pkgtmp" "{\"target_heimdall\":\"$TARGET_HEIMDALL\"}" > /dev/null 2>&1; then
    rm -rf "$pkgtmp"
    fail "could not deploy span-fault proxy"
    return 1
  fi
  rm -rf "$pkgtmp"
  ...
  wait_until "$OBSERVE_S" _span_failing ||
    {
      warn "no span-fetch failure observed; the repoint/proxy did not engage"
      info "=== RESULT: INCONCLUSIVE (proxy did not engage) ==="
      return 0
    }
