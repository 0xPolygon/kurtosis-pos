...
# Deploy pos-portal L2 (child) contracts, then do the final cross-chain wiring on L1
...
echo "Deploying pos-portal L2 (child) contracts..."
export DEPLOYER_PRIVATE_KEY="${PRIVATE_KEY}"
forge script -vvvv --rpc-url "${L2_RPC_URL}" --broadcast --legacy \
  scripts/deployment-scripts/deployPosPortalChild.s.sol:DeployPosPortalChildScript
...
rcm_proxy=$(jq -r '.root.posPortal.RootChainManagerProxy' contractAddresses.json)
ccm_proxy=$(jq -r '.child.posPortal.ChildChainManagerProxy' contractAddresses.json)
...
# Ether "root" is the canonical sentinel; matches pos-portal's mainnet config.
ether_root="0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE"

for pair in \
    "ERC20:$(jq -r '.root.posPortal.DummyERC20' contractAddresses.json):$(jq -r '.child.posPortal.DummyERC20' contractAddresses.json):${erc20_type}" \
    "MintableERC20:$(jq -r '.root.posPortal.DummyMintableERC20' contractAddresses.json):$(jq -r '.child.posPortal.DummyMintableERC20' contractAddresses.json):${mintable_erc20_type}" \
    "ERC721:$(jq -r '.root.posPortal.DummyERC721' contractAddresses.json):$(jq -r '.child.posPortal.DummyERC721' contractAddresses.json):${erc721_type}" \
    "MintableERC721:$(jq -r '.root.posPortal.DummyMintableERC721' contractAddresses.json):$(jq -r '.child.posPortal.DummyMintableERC721' contractAddresses.json):${mintable_erc721_type}" \
    "ERC1155:$(jq -r '.root.posPortal.DummyERC1155' contractAddresses.json):$(jq -r '.child.posPortal.DummyERC1155' contractAddresses.json):${erc1155_type}" \
    "MintableERC1155:$(jq -r '.root.posPortal.DummyMintableERC1155' contractAddresses.json):$(jq -r '.child.posPortal.DummyMintableERC1155' contractAddresses.json):${mintable_erc1155_type}" \
    "Ether:${ether_root}:$(jq -r '.child.posPortal.MaticWETH' contractAddresses.json):${ether_type}"; do
...
if [[ -s "${CONTRACT_ADDRESSES_FILE}" ]]; then
  echo "pos-portal L2 deploy + cross-chain wiring complete. Final contractAddresses.json:"
