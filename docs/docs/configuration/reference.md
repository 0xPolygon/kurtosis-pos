...
| cl_image | string | sigp/lighthouse:v8.2.2     | Image for the CL client          |
...
| cl_image                         | string | 0xpolygon/heimdall-v2:0.11.0 | Image for the CL client                                                                                                                                                                                                                                                                                                                             |
| cl_queue_image                   | string | rabbitmq:4.3.5               | Image for the CL queue                                                                                                                                                                                                                                                                                                                              |
...
| el_image                         | string | 0xpolygon/bor:2.10.1         | Image for the EL client (bor: `0xpolygon/bor:2.10.1`)                                                                                                                                                                                                                                                                                               |
...
| el_bor_accept_private_tx         | bool   | false                        | Block-producer side of the private-tx relay. When `true`, the node accepts `eth_sendRawTransactionPrivate` calls from a relayer (sets `[jsonrpc] accept-private-tx`). Typically set on validators. See `.github/configs/private-tx.yml`.                                                                                                            |
| el_bor_enable_private_tx_relay   | bool   | false                        | Relayer side of the private-tx feature. When `true`, the node accepts user-facing `eth_sendRawTransactionPrivate`, suppresses devp2p gossip for those hashes, and forwards each tx to the BPs listed in `el_bor_private_tx_bp_endpoints`. Reflects in the service name as `…-private-relay` for discovery.                                          |
| el_bor_private_tx_bp_endpoints   | list   | []                           | Optional override for `[relay] bp-rpc-endpoints`. When `el_bor_enable_private_tx_relay` is `true` and this list is empty, the launcher auto-fills it with the in-cluster RPC URL of every validator EL node.                                                                                                                                        |
| el_bor_extra_args                | list   | []                           | Escape hatch for arbitrary bor CLI flags not modelled as first-class fields. A list of already-split tokens appended verbatim to `bor server` (after `--config`, so they override `config.toml`), e.g. `["--p2p.nosnap", "--miner.disable-pending-block", "--grpc.addr", "0.0.0.0:3131"]`. Bor only; tokens with embedded spaces are not supported. |
...
| austin_fork_block                     | int    | 999999999          | Block number for Austin hard fork activation (bor >= 2.10.0). Disabled by default — post-Austin headers drop the TxDependency field, so bor < 2.10.0 halts at the boundary. Only set it on a single-version bor >= 2.10.0 devnet. |
