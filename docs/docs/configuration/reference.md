...
| el_bor_use_sequence_store        | bool   | false                        | Connect Bor to the enclave sequence store. Validators publish block events; RPC nodes consume them and relay preconfirmation transactions to producers. Requires a compatible Bor image; enabling it deploys the store (see `sequence_store_params`). |
...
| valencia_fork_block                   | int    | 128                | Block number for Valencia hard fork activation (bor >= 2.10.0)                                               |
