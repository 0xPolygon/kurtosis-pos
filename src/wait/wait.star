...
def wait_for_l2_startup(plan, cl_api_url, cl_type):
    ...
    plan.run_sh(
        ...
        run='while true; do sleep 5; echo "L2 Chain is starting up..."; SPAN_ID="$(curl -s "$CL_RPC_URL/{}" | jq -r "{}")"; echo "Span ID: $SPAN_ID"; if [ "$SPAN_ID" != "null" && "$SPAN_ID" != "0" ]; then echo "✅ L2 Chain has started!"; break; fi; done'.format(
            endpoint, json_path
        ),
        wait="300s",
    )
