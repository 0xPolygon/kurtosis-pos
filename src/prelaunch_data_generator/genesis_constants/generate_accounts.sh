...
jq --compact-output '.[]' eth_accounts.json | while read -r account; do
    ...
    heimdallcli generate-validatorkey --home "${heimdall_config_path}" --logs_writer_file logs.txt "${private_key}"
    cp ./priv_validator_key.json "${heimdall_config_path}"/config
    tendermint_public_key=$(heimdalld show-account --home "${heimdall_config_path}" --logs_writer_file logs.txt | jq --raw-output '.pub_key')
