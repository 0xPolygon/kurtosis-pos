...
def test_get_validator_data_single_validator(plan):
    ...
    expect.eq(
        result.accounts[0]["address"], "0x1234567890abcdef1234567890abcdef12345678"
    )
    expect.eq(result.accounts[0]["coins"][0]["denom"], "matic")
    expect.eq(
        result.accounts[0]["coins"][0]["amount"], "1000000000000000000000000000"
    )  # 10^9 + 18 zeros
    expect.eq(result.accounts[0]["sequence_number"], "0")
    expect.eq(result.accounts[0]["account_number"], "0")
    expect.eq(result.accounts[0]["module_name"], "")
    expect.eq(result.accounts[0]["module_permissions"], None)

    # Test dividends
    expect.eq(len(result.dividends), 1)
    expect.eq(result.dividends[0]["user"], "0x1234567890abcdef1234567890abcdef12345678")
    expect.eq(result.dividends[0]["feeAmount"], "0")

    # Test signing infos
    expect.eq(len(result.signing_infos), 1)
    expect.eq(result.signing_infos["1"]["valID"], "1")
    expect.eq(result.signing_infos["1"]["startHeight"], "0")
    expect.eq(result.signing_infos["1"]["indexOffset"], "0")

    # Test validator set
    expect.eq(len(result.validator_set), 1)
    expect.eq(result.validator_set[0]["ID"], "1")
    expect.eq(result.validator_set[0]["accum"], "0")
    expect.eq(result.validator_set[0]["endEpoch"], "0")
    ...
    expect.eq(result.validator_set[0]["power"], "1000000000")  # 10^9
    expect.eq(
        result.validator_set[0]["pubKey"],
        "0x04pubkey123456789012345678901234567890123456789012345678901234567890",
    )
    expect.eq(
        result.validator_set[0]["signer"], "0x1234567890abcdef1234567890abcdef12345678"
    )
    expect.eq(result.validator_set[0]["startEpoch"], "0")
...
def test_get_validator_data_multiple_validators(plan):
    ...
    expect.eq(len(result.signing_infos), 3)
    expect.eq(len(result.validator_set), 3)

    # Test that validator IDs are sequential
    for i in range(3):
        validator_id = str(i + 1)
        expect.eq(result.validator_set[i]["ID"], validator_id)
        expect.eq(result.signing_infos[validator_id]["valID"], validator_id)

    # Test that addresses are correctly set
    for i in range(3):
        expected_address = "0x{}234567890abcdef1234567890abcdef12345678".format(i)
        expect.eq(result.accounts[i]["address"], expected_address)
        expect.eq(result.dividends[i]["user"], expected_address)
        expect.eq(result.validator_set[i]["signer"], expected_address)
...
def test_get_validator_data_empty_list(plan):
    ...
    expect.eq(len(result.signing_infos), 0)
...
def test_get_validator_data_structure_consistency(plan):
    ...
    expect.eq(hasattr(result, "signing_infos"), True)
    ...
    required_dividend_fields = ["user", "feeAmount"]
    for field in required_dividend_fields:
        expect.eq(field in dividend_item, True)

    # Verify signing info structure
    signing_info_item = result.signing_infos["1"]
    required_signing_info_fields = ["valID", "startHeight", "indexOffset"]
    for field in required_signing_info_fields:
        expect.eq(field in signing_info_item, True)
